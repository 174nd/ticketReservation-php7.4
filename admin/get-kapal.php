...
} else if ($_POST['set'] == 'get_rute') {
  $set_rute = array();
  $sql1 = mysqli_query($conn, "SELECT *,(SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_awal) AS pelabuhan_awal, (SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_akhir) AS pelabuhan_akhir FROM rute WHERE id_jadwal='$_POST[id_jadwal]'");
  for ($i = 0; $Data = mysqli_fetch_assoc($sql1); $i++) {
    $set_rute[] = [
      'id_rute' => $Data['id_rute'],
      'pelabuhan_awal' => $Data['pelabuhan_awal'],
      'pelabuhan_akhir' => $Data['pelabuhan_akhir'],
...
} else if ($_POST['set'] == 'simpan_rute') {
  if (setInsert(['pelabuhan_awal' => $_POST['pelabuhan_awal'], 'pelabuhan_akhir' => $_POST['pelabuhan_akhir'], 'harga_tiket' =>  str_replace('.', '', $_POST['harga_tiket']), 'id_jadwal' => $_POST['id_jadwal']], 'rute')) {
    $set_rute = array();
    $sql1 = mysqli_query($conn, "SELECT *,(SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_awal) AS pelabuhan_awal, (SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_akhir) AS pelabuhan_akhir FROM rute WHERE id_jadwal='$_POST[id_jadwal]'");
    for ($i = 0; $Data = mysqli_fetch_assoc($sql1); $i++) {
      $set_rute[] = [
...
} else if ($_POST['set'] == 'hapus_rute') {
  if (setDelete('rute', ['id_jadwal' => $_POST['id_jadwal']])) {
    $set_rute = array();
    $sql1 = mysqli_query($conn, "SELECT *,(SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_awal) AS pelabuhan_awal, (SELECT nm_pelabuhan from pelabuhan WHERE pelabuhan.id_pelabuhan=rute.pelabuhan_akhir) AS pelabuhan_akhir FROM rute WHERE id_jadwal='$_POST[id_jadwal]'");
    for ($i = 0; $Data = mysqli_fetch_assoc($sql1); $i++) {
      $set_rute[] = [
